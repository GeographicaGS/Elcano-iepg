...
from common.errorhandling import ElcanoApiRestError, DataValidator
...
#         self.queryCommit(q, [id_document, hash])
    ...
    def __createPdf(self, lang, name, hash, idDocument):
...
#         return id_document
    ...
    def searchInAuthors(self, search):
        """Returns a set with document ID that complies to the search criteria in authors."""
        sql = """
        select 
        array_agg(id_document) as id
        from 
        www.author
        where
        """
        ...
        for s in search.split(","):
            sql += """
            (name ilike %s or
            position_en ilike %s or
            position_es ilike %s or
            twitter_user ilike %s) or
            """
            bi.extend(["%"+s+"%","%"+s+"%","%"+s+"%","%"+s+"%"])
        sql = sql.rstrip(" or\n")+";"

        m = self.query(sql, bi).row()["id"]
        if m:
            return(set(m))
        else:
            return(set([]))
    ...
    def searchInLabels(self, search):
        """Returns a set with document ID that complies to the search criteria in labels."""
        sql = """
        with a as(
        select
        id_document as id
        from
        www.document_label_en a inner join
        www.label_en b on
        a.id_label_en=b.id_label_en
        where
        """
        bi = []
        for s in search.split(","):
        ...
        id_document as id
        from
        www.document_label_es a inner join
        www.label_es b on
        a.id_label_es=b.id_label_es
        where
        """
        for s in search.split(","):
            sql += """
    ...
    def searchInPdf(self, search):
        """Returns a set with document ID that complies to the search criteria in PDF."""
        sql = """
        select
        array_agg(id_document) as id
        from
...
#         return self.query(q, bindings).result()
    ...
    def __attachLabel(self, idLabel, idDocument, lang):
        """Attaches a label ID for a language to a document."""
        dv = DataValidator()
        dv.checkLang(lang)
        dv.checkNumber(idLabel)
        dv.checkNumber(idDocument)
        self.insert("www.document_label_"+lang,
                    {"id_document": idDocument,
                     "id_label_"+lang: idLabel})


    def __createAuthor(self, data, idDocument):
        """Attaches an author to a document.
        TODO: fix the author dilemma once and for all!"""
        DataValidator().checkNumber(idDocument)
