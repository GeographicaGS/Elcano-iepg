...
# # from backend.utils import prettyNumber
# # import config
...
def newDocument():
    ...
    if [j["title_es"], j["title_en"], j["theme_es"], j["theme_en"], 
        ...
        j["description_es"], j["description_en"]]==\
        ["","","","","",""]:
        raise ElcanoApiRestError("Invalid document.", status=200, payload=j)
...
def movePdfFile(hash):
    ...
    try:
        ...
        raise ElcanoApiRestError("File not found.", status=200, 
                                 payload={"hash": hash})
...
def getDocumentList():
    ...
    app.logger.info(m.searchInAuthors("powell,olivié"))
    app.logger.info(m.searchInLabels("españa,spain,energy,política"))
    app.logger.info(m.searchInPdf("economía,elcano"))

    # totalSize = m.getDocumentListSize(search=fsearch)
